...
if 'cfg_curriculum' not in st.session_state:
    st.session_state.cfg_curriculum = pd.DataFrame([{
        ...
        "Аудиторія": "✨ Автоматичний подбір з фонду"
    }])
...
def generate_fast_schedule():
        ...
        for d in range(days_count):
            for g_name in active_groups:
                ...
                if avoid_windows and slots_count >= 3:
                    for s1 in range(slots_count - 2): model.Add(g_vars[s1] + g_vars[s1+2] - g_vars[s1+1] <= 1)
            for s in specs:
    ...
    if status not in [OPTIMAL, FEASIBLE]: return None, "Неможливо створити розклад."
...
if st.session_state.schedule_data:
    ...
    if st.button("📥 Завантажити повний розклад у Excel"):
        output = io.BytesIO()
        with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
            ...
            # 1. Листи по тижнях (для студентів)
            for w in range(1, data["max_weeks"] + 1):
                sheet_data = []
                for d in ACTIVE_DAYS:
                    for sl in range(slots_count):
                ...
                sheet_name = f"Тиждень {w}"
                df_w.to_excel(writer, sheet_name=sheet_name, index=False)
            ...
            # 2. Листи для викладачів (тижні в стовпцях)
            for t in data["active_teachers"]:
                ...
                # Очищення імені листа від заборонених символів Excel
        ...
        st.download_button(
            label="Натисніть тут, щоб зберегти файл",
